...
export default function EventosPage() {
  ...
  return (
    <div className="flex gap-6 h-[calc(100vh-120px)]">
      ...
      <div className={`flex-1 space-y-6 ${selectedEvento ? 'lg:mr-80' : ''}`}>
        ...
        <div className="card p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div className="flex items-center gap-2 sm:gap-4 justify-center sm:justify-start">
              ...
              <h2 className="text-lg sm:text-xl font-semibold text-[#3C3C3C] min-w-[140px] sm:min-w-[180px] text-center">
            ...
            <button onClick={goToToday} className="btn-secondary flex items-center gap-2 justify-center sm:justify-start">
              <Calendar size={16} />
              Hoy
            </button>
          </div>

          {/* Headers de días - desktop */}
          <div className="hidden sm:grid grid-cols-7 gap-2 mb-2">
            {DAYS_SHORT.map((day, index) => (
              <div key={`header-${index}`} className="text-center text-sm font-semibold text-[#6B7280] py-2">
                {day}
              </div>
            ))}
          </div>

          {/* Headers de días - mobile */}
          <div className="grid sm:hidden grid-cols-7 gap-1 mb-2">
            {DAYS_MOBILE.map((day, index) => (
              <div key={`header-mobile-${index}`} className="text-center text-xs font-semibold text-[#6B7280] py-1">
                {day}
              </div>
            ))}
          </div>
                ...
                {calendarDays.map((item, index) => {
                  ...
                  return (
                    <div
                      key={`day-${index}`}
                      className={`
                        min-h-[120px] p-3 rounded-lg border transition-all
                        ${day ? 'bg-white hover:shadow-md' : 'bg-gray-50'}
                        ${isToday(day) ? 'border-[#FF6B35] border-2' : 'border-[#E8E8E8]'}
                      `}
                    >
                      {day && (
                        <>
                          <div className={`text-sm font-medium mb-2 ${isToday(day) ? 'text-[#FF6B35]' : 'text-[#6B7280]'}`}>
                            {day}
                          </div>
                          ...
                          <div className="space-y-1">
                            {eventosDelDia.map(evento => (
                              <div
                                ...
                                className="text-xs p-2 rounded-md bg-[#FF6B35] text-white hover:bg-[#E85A2B] transition-colors cursor-pointer truncate"
                              >
                ...
                {calendarDays.map((item, index) => {
                  ...
                  return (
                    <div
                      ...
                      className={`
                        aspect-square p-1 rounded-lg border transition-all text-center flex flex-col items-center justify-center
                    ...
                    >
                      {day && (
                        <>
                          <div className={`text-xs ${isToday(day) ? 'text-[#FF6B35] font-bold' : 'text-[#6B7280]'}`}>
                            {day}
                          </div>
                          {hasEvents && (
                            <div className="w-1.5 h-1.5 rounded-full bg-[#FF6B35] mt-1" />
                          )}
                        </>
                      )}
                    </div>
      ...
      {selectedEvento && (
        <>
          ...
          {/* Panel lateral */}
          <div className={`
            fixed lg:fixed top-0 right-0 h-screen w-80 bg-white border-l border-[#E8E8E8] z-50
            ...
            <div className="flex flex-col h-full p-6">
              ...
              <div className="flex-1 space-y-6">
                ...
                <div>
                  ...
                  <p className="text-2xl font-semibold text-[#3C3C3C]">
                    {selectedEvento.cantidadInvitados}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-lg font-semibold text-[#3C3C3C]">
                    USD {selectedEvento.precioPorInvitado.toFixed(0)}
                  </p>
                </div>
              </div>
