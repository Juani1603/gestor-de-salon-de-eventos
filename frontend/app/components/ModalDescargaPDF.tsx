...
// react-pdf no funciona en SSR, se importa dinámicamente
...
// Importamos el documento PDF también dinámicamente
const PlanificacionPDF = dynamic(
    () => import('./PlanificacionPDF'),
    { ssr: false, loading: () => <PdfLoadingPlaceholder /> }
);

function PdfLoadingPlaceholder() {
    return (
        <div className="flex-1 flex flex-col items-center justify-center gap-3 bg-[#FAFAFA] rounded-xl">
            <Loader2 size={24} className="text-[#FF6B35] animate-spin" />
            <p className="text-sm text-[#9CA3AF]">Preparando previsualización...</p>
        </div>
    );
}
...
export default function ModalDescargaPDF({ evento, planificacion, onClose }: ModalDescargaPDFProps) {
    ...
    return (
        ...
        >
            <div
                className={`bg-white rounded-3xl shadow-2xl flex flex-col transition-all duration-300 ${vistaPrevia ? 'w-full max-w-3xl h-[92vh]' : 'max-w-md w-full'
                    }`}
                onClick={(e) => e.stopPropagation()}
                style={{ animation: 'modalIn 0.2s ease-out' }}
            >
                {/* Header */}
                <div className="px-7 pt-7 pb-5 border-b border-[#F5F5F5] shrink-0 flex items-center justify-between">
                    <div>
                        ...
                        <div className="bg-[#FAFAFA] border border-[#F0F0F0] rounded-xl p-4 space-y-3">
                            ...
                            <div className="space-y-2">
                                {[
                                    ...
                                    { label: 'Fecha', valor: new Date(evento.fechaEvento.split('T')[0] + 'T00:00:00').toLocaleDateString('es-ES', { day: '2-digit', month: 'long', year: 'numeric' }) },
                                    { label: 'Horarios', valor: `Llegada ${planificacion.horaLlegada}hs · Cena ${planificacion.horaComida}hs${planificacion.horaSalida ? ` · Cierre ${planificacion.horaSalida}hs` : ''}` },
                ...
                {/* Vista previa */}
                {vistaPrevia && (
                    <div className="flex-1 px-7 py-4 min-h-0">
                        <PDFViewer width="100%" height="100%" style={{ border: 'none', borderRadius: '12px' }}>
                            {/* @ts-ignore — carga dinámica */}
                            <PlanificacionPDF evento={evento} planificacion={planificacion} />
                        </PDFViewer>
                    </div>
                )}
                ...
                <div className="px-7 pb-7 pt-4 border-t border-[#F5F5F5] shrink-0 flex gap-3">
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-[#EBEBEB] text-sm font-semibold text-[#6B7280] hover:bg-[#F9F9F9] transition-colors"
                    >
                        <FileText size={14} />
                    ...
                    <button
                        onClick={async () => {
                            const PlanificacionPDFComp = (await import('./PlanificacionPDF')).default;
                            const blob = await pdf(<PlanificacionPDFComp evento={evento} planificacion={planificacion} />).toBlob();
                        ...
                        className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold text-white transition-all hover:shadow-lg hover:shadow-orange-200 active:scale-95"
